...
import com.github.nokamoto.webpush.WebpushTestingServiceSpec.WebpushTestingServiceTest
...
class WebpushTestingServiceSpec extends FlatSpec {
  ...
  def test(driver: String): Unit = {
    "WebpushClient" should s"send a message to the $driver push service" taggedAs WebpushTestingServiceTest in {
      val okHttpClient = new OkHttpClient()
      val empty = RequestBody.create(null, "")

      val startSuite = okHttpClient
        .newCall(
          new Request.Builder().url(s"$testingUrl/$driver").post(empty).build())
        .execute()
...
object WebpushTestingServiceSpec {
  object WebpushTestingServiceTest
      extends Tag("com.github.nokamoto.webpush.WebpushTestingServiceTest")
